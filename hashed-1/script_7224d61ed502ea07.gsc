...
#namespace cschashed-3\script_1611421ee9b880d3;
...
function add_dynamic_wallbuy(weapon, wallbuy, pristine)
{
	...
	{
		if(pristine || cschashed-2\script_709bf7c56eb65adf::is_placeable_mine(weapon))
		{
...
function wall_weapon_update_prompt(player)
{
	...
	if(!player_has_weapon)
	{
		...
		else if(player bgb::is_enabled(#"hash_4a6b297c85fafec1"))
		{
			if(function_8b1a219a())
			{
	...
	else if(player function_284616f8())
	{
	...
	var_f7b97cc4 = 0;
	var_60da4cc1 = player zm_weapons::get_upgrade_weapon(weapon, 1);
	if(player zm_utility::function_aa45670f(weapon, 0) || player zm_utility::function_aa45670f(var_60da4cc1, 0))
	{
...
{
	if(isdefined(w_weapon) && w_weapon == self cschashed-2\script_709bf7c56eb65adf::get_player_placeable_mine())
	{
		return 0;
	}
	return 1;
...
function weapon_spawn_think()
{
	...
	if(isdefined(self.stub) && (isdefined(self.stub.trigger_per_player) && self.stub.trigger_per_player))
	{
		...
		if(cschashed-2\script_709bf7c56eb65adf::is_placeable_mine(self.weapon))
		{
	...
	if(is_grenade || cschashed-2\script_709bf7c56eb65adf::is_melee_weapon(self.weapon))
	{
		self.first_time_triggered = 0;
	...
	for(;;)
	{
		...
		if(player cschashed-2\script_709bf7c56eb65adf::has_powerup_weapon())
		{
			wait(0.1);
			continue;
		}
		...
		if(!player_has_weapon)
		{
			if(zm_utility::function_e05ac4ad(player, cost))
			{
				...
				else if(cschashed-2\script_709bf7c56eb65adf::is_lethal_grenade(self.weapon))
				{
					player zm_weapons::weapon_take(player cschashed-2\script_709bf7c56eb65adf::get_player_lethal_grenade());
					player cschashed-2\script_709bf7c56eb65adf::set_player_lethal_grenade(self.weapon);
				}
				weapon = self.weapon;
				...
				if(isdefined(weapon))
				{
					...
					bb::function_ab9bb12c(player, self, cost, weapon.name, player zm_weapons::has_upgrade(weapon), "_weapon", "_purchase");
		...
		else
		{
			...
			else if(zm_utility::function_e05ac4ad(player, ammo_cost))
			{
				...
				if(ammo_given)
				{
					...
					bb::function_ab9bb12c(player, self, ammo_cost, weapon.name, var_47834995, "_ammo", "_purchase");
					weaponindex = undefined;
					if(isdefined(weapon))
					{
						weaponindex = matchrecordgetweaponindex(weapon);
...
function show_all_weapon_buys(player, cost, ammo_cost, is_grenade)
{
	...
	is_melee = cschashed-2\script_709bf7c56eb65adf::is_melee_weapon(self.weapon);
	if(isdefined(model))
	{
