...
#using hashed-3\ct_shared_desc.gsc;
#using hashed-1\luielemtext.gsc;
#using hashed-2\luielembar_ct.gsc;
#using hashed-1\ct_shared_warning.gsc;
#using hashed-1\ct_shared_ingame_hint.gsc;
...
#using hashed-3\swat_team.gsc;
#using script_788472602edbe3b9;
#using script_7c4b080c0766fedc;
#using script_bb99a1f9be8d0a7;
#using hashed-3\ct_shared_objcounter.gsc;
...
function function_8963dae3()
{
	...
	self endon(#"hash_71db431c3e2ea07d", #"keyline_off");	if(-1)
	{
		wait(0.5);
...
{
	self endon_callback(&function_c4f8317a, #"death", #"keyline_off");	self notify(#"hash_71db431c3e2ea07d");
	waitframe(1);
...
{
	level endon(#"show_aar");	self function_a6ee046();
...
{
	level endon(#"show_aar");	if(!isdefined(level.var_c48d861d))
	{
		level.var_c48d861d = [];
		waitframe(1);
...
{
	level endon_callback(&function_1386d240, #"show_aar");	if(isdefined(level.var_c48d861d))
	{
		while(true)
		{
...
{
	level endon(#"hardpoint_complete");	self waittill(#"death");
...
function function_1ca79c02(str_volume, var_7a037bc3, var_224fccce, var_c088c2dd)
{
	...
	level notify(#"hardpoint_complete");	level.b_hardpoint_complete = 1;
}
	...
	if(var_4b3a6521 !== self.var_f82509cd)
	{
		...
		switch(var_4b3a6521)
		{
			...
			case "download_complete":			{
				level.ct_progressbar_status ct_progressbar_status::set_state(self, #"download_complete");				wait(3);
...
function function_af70d00(n_type, var_9d99d28, var_e25eca94, var_66c4c2c6, var_c7165970)
{
	...
	if(!isdefined(level.var_a76df030[var_b1cb18f1][str_character][var_d0c7d280]))
	{
		...
		level.var_a76df030[var_b1cb18f1][str_character][var_d0c7d280][2] = n_silver;
		level.var_a76df030[var_b1cb18f1][str_character][var_d0c7d280][3] = n_gold;
...
function function_5dec7b34(str_warning, var_3608d414)
{
	...
	if(isdefined(level.var_820c5561))
	{
		...
		self.var_ef34d22c[#"cycle1"] = hash(var_fe0e3b3a + "_cycle1");		self.var_ef34d22c[#"cycle2"] = hash(var_fe0e3b3a + "_cycle2");		self.var_ef34d22c[#"fadeout"] = hash(var_fe0e3b3a + "_fadeout");
...
function function_3915e4f9()
{
	...
	while(true)
	{
		self function_f05e5477(hash(self.var_ef34d22c[#"cycle1"]));		self playsound(#"mpl_ui_timer_countdown");
		wait(1);
		self function_f05e5477(hash(self.var_ef34d22c[#"cycle2"]));		self playsound(#"mpl_ui_timer_countdown");
		wait(1);
	}
}
...
function function_68e9fa9e(str_label, var_8f1f8855, n_total, n_mode = 0, var_455d8dee = 1)
{
	if(isplayer(self))
	{
		...
		if(var_455d8dee)
		{
			level.var_a3e856b1[#"init"] = #"total_init";			level.var_a3e856b1[#"idle"] = #"total";
...
{
	if(isstring(var_65ad93bb) || function_7a600918(the_weapon))
	{
...
function function_e96cc63f(n_count = 3, var_d7172908, var_810b40e6 = 1, var_4837ee77 = 1, var_32df2a60 = 1, b_randomize = 0, str_targetname = "fillspawn_enemies")
{
	...
	while(true)
	{
		...
		while(a_enemies.size < n_count)
		{
			...
			namespace_64a487a9::activate_bots(n_to_spawn, #"axis", str_targetname);
	...
	while(true)
	{
		...
		if(n_to_spawn > 0)
		{
			...
			if(level.var_e4db2e78.size > 0)
			{
				...
				{
					namespace_64a487a9::activate_bots(1, #"axis", str_targetname);
					wait(var_65c9209b);
				}
			}
		}
...
function function_ffb2833a()
{
	...
	if(var_e2a6292f.size > 0)
	{
		...
		{
			if(!(isdefined(var_247c990.var_c037e962) && var_247c990.b_initialized))
			{
...
function colbounds_on(str_id)
{
	...
	{
		var_e50687db solid();
		if(isdefined(level.var_85697102[str_id].var_314be19c) && level.var_85697102[str_id].b_disconnectpaths)
		{
...
function function_95f28639(str_id)
{
	...
	#/
	if(level.var_85697102[str_id].a_effects.size === 0)
	{
		level.var_85697102[str_id].a_effects = [];
		foreach(var_e50687db in level.var_85697102[str_id].var_e62fe646)
		{
			if(isdefined(var_e50687db.target))
			{
				...
				level.var_85697102[str_id].var_1585e0a4[level.var_85697102[str_id].a_effects.size] = spawnfx(str_effect, s_loc.origin, fwd, right);
				triggerfx(level.var_85697102[str_id].var_1585e0a4[level.var_85697102[str_id].a_effects.size - 1]);
			}
		}
	}
}
...
function function_a3f8ddfa(str_id)
{
	/#
		assert(isdefined(level.var_85697102[str_id]), "");
	#/
	if(isdefined(level.var_85697102[str_id].var_1585e0a4) && level.var_85697102[str_id].a_effects.size > 0)
	{
		foreach(effect in level.var_85697102[str_id].a_effects)
		{
			effect delete();
		}
		level.var_85697102[str_id].a_effects = [];
