...
function stoker_death_explosion(localclientnum, oldvalue, newvalue, bnewent, binitialsnap, fieldname, wasdemojump)
{
	switch(newvalue)
	{
		...
		case 2:
		{
			self notify(#"unlock_model");			v_origin = self gettagorigin("j_shoulder_le");
...
private function function_d58cd2d5(localclientnum)
{
	...
	var_19253097 = createdynentandlaunch(localclientnum, "c_t8_zmb_titanic_stoker_upperarm1_gibbed", var_99625f1a, var_cf1365cd, var_99625f1a, var_4423d870 * randomfloatrange(0.6, 1.2));
	wait(0.1);
...
{
	self waittill_timeout(10, #"death", #"unlock_model");	util::unlock_model(model);
}
