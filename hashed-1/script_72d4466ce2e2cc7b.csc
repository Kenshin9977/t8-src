...
#namespace cschashed-2\script_72d4466ce2e2cc7b;
...
function function_a78bbf22(localclientnum)
{
	...
	while(!(isdefined(level.gameended) && level.gameended))
	{
		for(ga = 0; ga < a_keys.size; ga++)
		{
			...
			if(isdefined(gadgetpower) && gadgetpower == 1 && (isdefined(self.var_9623f1d5[var_9adc5613]) && !self.var_9623f1d5[ga]))
			{
...
function function_88dc3170(localclientnum)
{
	...
	while(!(isdefined(level.gameended) && level.gameended))
	{
		for(i = 0; i < var_fdbf663f.size; i++)
		{
			...
			var_59015958 = getuimodelvalue(_the_dad16f153c7d14b);
